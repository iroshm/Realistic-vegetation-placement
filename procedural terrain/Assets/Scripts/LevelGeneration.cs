...
public class LevelGeneration : MonoBehaviour
{
    ...
    [SerializeField]
    public int levelWidthInTiles, levelDepthInTiles; // nuber of width tiles and height tiles.
    ...
    {
        print(levelWidthInTiles);
        // get the tile dimensions from the tile Prefab  ***** tile dimension
        Vector3 tileSize = tilePrefab.GetComponent<MeshRenderer>().bounds.size;
        int tileWidth = (int)tileSize.x;
        int tileDepth = (int)tileSize.z;

        // calculate the number of vertices of the tile in each axis using its mesh ****** tile exis vertices number
        ...
        // build an empty LevelData object, to be filled with the tiles to be generated****** empty leveldata obj, <- fill with tiles
        LevelData levelData = new LevelData(tileDepthInVertices, tileWidthInVertices, this.levelDepthInTiles, this.levelWidthInTiles);

        // for each Tile, instantiate a Tile in the correct position************* tile instantiation
